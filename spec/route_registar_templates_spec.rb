    ...
    describe 'config/registrar_settings.json' do
      ...
      describe 'when given a valid set of properties' do
        it 'renders the template' do
          rendered_hash = JSON.load(template.render(merged_manifest_properties, consumes: links))
      ...
      describe 'when tls is not enabled and the san is not provided' do
        ...
        it 'renders the template' do
          expect { template.render(merged_manifest_properties, consumes: links) }.not_to raise_error(RuntimeError)
        end
      end
    end
  end
