...
var _ = Describe("RouteServicesServer", func() {
	...
	Describe("GetRoundTripper", func() {
		It("applies TLS version and cipher settings to the client config", func() {
			...
			rssClient, err := router.NewRouteServicesServer(cfg)
			Expect(err).NotTo(HaveOccurred())
			defer rssClient.Stop()

			clientCfg := rssClient.GetRoundTripper().TLSClientConfig()
			Expect(clientCfg.MinVersion).To(Equal(tls.VersionTLS12))
			Expect(clientCfg.MaxVersion).To(Equal(tls.VersionTLS13))
