...
func (m *CertificateProperties) XXX_DiscardUnknown() {
	xxx_messageInfo_CertificateProperties.DiscardUnknown(m)
}
...
var fileDescriptor_9291b57c1fe01997 = []byte{
	...
	0xf5, 0x0a, 0x8a, 0xf2, 0x4b, 0xf2, 0x85, 0xd8, 0x72, 0xf3, 0x53, 0x52, 0x73, 0x8a, 0x95, 0x3c,
	0xb8, 0x44, 0x9d, 0x11, 0xea, 0x02, 0xe0, 0xca, 0x84, 0xf4, 0xb9, 0x84, 0xf3, 0x8b, 0xd2, 0x13,
	0xf3, 0x32, 0xab, 0x12, 0x4b, 0x32, 0xf3, 0xf3, 0x12, 0x73, 0xe2, 0x4b, 0xf3, 0x32, 0x4b, 0x24,
	0x18, 0x15, 0x98, 0x35, 0x38, 0x83, 0x84, 0x50, 0xa5, 0x42, 0xf3, 0x32, 0x4b, 0x9c, 0x4c, 0x2e,
	...
	0x72, 0x0c, 0x37, 0x1e, 0xcb, 0x31, 0x24, 0xb1, 0x81, 0x9d, 0x63, 0x0c, 0x08, 0x00, 0x00, 0xff,
	0xff, 0x07, 0xe2, 0x02, 0xdf, 0xae, 0x00, 0x00, 0x00,
}
...
func (this *CertificateProperties) GoString() string {
	...
	s := make([]string, 0, 5)
	s = append(s, "&models.CertificateProperties{")
	s = append(s, "OrganizationalUnit: "+fmt.Sprintf("%#v", this.OrganizationalUnit)+",\n")
	s = append(s, "}")
	return strings.Join(s, "")
}
func valueToGoStringCertificateProperties(v interface{}, typ string) string {
...
func (m *CertificateProperties) Size() (n int) {
	...
	if len(m.OrganizationalUnit) > 0 {
		for _, s := range m.OrganizationalUnit {
			l = len(s)
			n += 1 + l + sovCertificateProperties(uint64(l))
		}
	}
	return n
}
...
func (m *CertificateProperties) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		for shift := uint(0); ; shift += 7 {
			if shift >= 64 {
				return ErrIntOverflowCertificateProperties
			}
			if iNdEx >= l {
		...
		default:
			iNdEx = preIndex
			skippy, err := skipCertificateProperties(dAtA[iNdEx:])
			if err != nil {
				return err
			}
